 ...
 * @returns {string}
 */
function getIndentation(size = 0, type: IndentationTypes = IndentationTypes.SPACES): string {
 ...
 * @returns {string}
 */
export function indent(content = '', size = 1, type: IndentationTypes = IndentationTypes.SPACES): string {
  if (size < 1) {
    return content;
  }
