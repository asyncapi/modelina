...
const tsFiles = ['**/*.ts', '**/*.tsx', 'netlify/*.ts'];

export default [
  {
    ignores: ['node_modules/**', '.next/**', 'out/**', 'build/**', '**/*.css', 'next-env.d.ts']
  },
    ...
    rules: {
      'max-len': [
        'warn',
        {
          ...
          ignoreUrls: true,
          // Ignore classnames
          ignorePattern: "(className=\\{[\\s\\S]*\\}|.*\\}|'.*'|className='.*')"
        }
      ]
    }
  },
  {
    ...
    rules: {
      ...
      'import-x/no-extraneous-dependencies': 'off', // Website imports the local @asyncapi/modelina build via a tsconfig path alias, not a node_modules dependency
