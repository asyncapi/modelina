...
import { NO_NUMBER_START_CHAR, NO_DUPLICATE_ENUM_KEYS, NO_EMPTY_VALUE, NO_RESERVED_KEYWORDS, FormatHelpers, EnumKeyConstraint, EnumValueConstraint} from '../../../helpers';
...
export function defaultEnumValueConstraints(): EnumValueConstraint {
  return ({enumValue}) => {
    ...
    case 'string': return `"${enumValue}"`;
    case 'boolean':
    case 'bigint':
    case 'number': return enumValue;
    case 'object': return `"${JSON.stringify(enumValue).replace(/"/g, '\\"')}"`;
    default: return `"${JSON.stringify(enumValue)}"`;
    }
  };
}
