...
import { ConstrainedObjectModel, ConstrainedDictionaryModel, ConstrainedReferencedModel, ConstrainedMetaModel, ConstrainedEnumModel } from '../../../models';
...
function renderMarshalProperty(modelInstanceVariable: string, model: ConstrainedMetaModel) {
  if (model instanceof ConstrainedReferencedModel && !(model.referencedModel instanceof ConstrainedEnumModel) ) {
...
function renderMarshalProperties(model: ConstrainedObjectModel) {
  ...
  const marshalNormalProperties = normalProperties.map(([prop, propModel]) => {
    ...
    const propMarshalCode = renderMarshalProperty(modelInstanceVariable, propModel);
  ...
  const marshalUnwrapDictionaryProperties = normalProperties.map(([prop, propModel]) => {
    const modelInstanceVariable = 'value';
    const patternPropertyMarshalCode = renderMarshalProperty(modelInstanceVariable, propModel);
...
function renderUnmarshalProperties(model: ConstrainedObjectModel) {
  ...
  const unmarshalNormalProperties = normalProperties.map(([prop, propModel]) => {
    ...
    const unmarshalCode = renderUnmarshalProperty(modelInstanceVariable, propModel);
    return `if (${modelInstanceVariable} !== undefined) {
