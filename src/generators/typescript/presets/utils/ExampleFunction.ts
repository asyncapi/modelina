import { ConstrainedArrayModel, ConstrainedBooleanModel, ConstrainedEnumModel, ConstrainedFloatModel, ConstrainedIntegerModel, ConstrainedMetaModel, ConstrainedObjectModel, ConstrainedReferencedModel, ConstrainedStringModel, ConstrainedTupleModel, ConstrainedUnionModel } from '../../../../models';
...
export function renderValueFromModel(model: ConstrainedMetaModel, renderer: ClassRenderer): string | undefined {
  ...
  } else if (model instanceof ConstrainedReferencedModel) {
    return `${model.type}.example()`;
  } else if (model instanceof ConstrainedReferencedModel) {
    return `${model.type}.example()`;
  } else if (model instanceof ConstrainedUnionModel && model.unionModels.length > 0) {
    //Greedy example
    return renderValueFromModel(model.unionModels[0], renderer);
  } else if (model instanceof ConstrainedArrayModel) {
    const arrayType = renderValueFromModel(model.valueModel, renderer);
    return `[${arrayType}]`;
  } else if (model instanceof ConstrainedTupleModel && model.tupleModels.length > 0) {
    const values = model.tupleModels.map((tupleModel) => {
