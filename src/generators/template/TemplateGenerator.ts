...
export class TemplateGenerator extends AbstractGenerator<TemplateOptions, TemplateRenderCompleteModelOptions> {
   ...
   */
  static getTemplateOptions(options?: DeepPartial<TemplateOptions>): TemplateOptions {
    const optionsToUse = mergePartialAndDefault(TemplateGenerator.defaultOptions, options) as TemplateOptions;
    //Always overwrite the dependency manager unless user explicitly state they want it (ignore default temporary dependency manager)
    if (options?.dependencyManager === undefined) {
      optionsToUse.dependencyManager = () => { return new TemplateDependencyManager(optionsToUse); };
    }
    return optionsToUse;
  }
  ...
  getDependencyManager(options: TemplateOptions): TemplateDependencyManager {
    return this.getDependencyManagerInstance(options) as TemplateDependencyManager;
  }
  ...
  constrainToMetaModel(model: MetaModel, options: DeepPartial<TemplateOptions>): ConstrainedMetaModel {    
    const optionsToUse = TemplateGenerator.getTemplateOptions({...this.options, ...options});
...
${outputModel.result}`; 
  ...
  async renderClass(model: ConstrainedObjectModel, inputModel: InputMetaModel, options?: Partial<TemplateOptions>): Promise<RenderOutput> {
    const optionsToUse = TemplateGenerator.getTemplateOptions({...this.options, ...options});
    const dependencyManagerToUse = this.getDependencyManager(optionsToUse);
  ...
  async renderEnum(model: ConstrainedEnumModel, inputModel: InputMetaModel, options?: Partial<TemplateOptions>): Promise<RenderOutput> {
    const optionsToUse = TemplateGenerator.getTemplateOptions({...this.options, ...options});
    const dependencyManagerToUse = this.getDependencyManager(optionsToUse);
