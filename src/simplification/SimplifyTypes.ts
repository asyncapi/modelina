 ...
 * 
 * @param input to find the simplified types for
 */
export default function simplifyTypes(input: Schema | boolean): Output {
  ...
  const callback = (schema: Schema | boolean): Output => {
    ...
    if (schema.then) {
      addToTypes(callback(schema.then));
    }
    if (schema.else) {
      addToTypes(callback(schema.else));
    }
    ...
    if (schema.not) {
      let notTypes = callback(schema.not);
