...
export function convertToArrayModel(jsonSchemaModel: CommonModel, name: string, alreadySeenModels: Map<CommonModel, MetaModel>): ArrayModel | undefined {
  ...
  if (isNormalArray) {
    ...
    const metaModel = new ArrayModel(name, jsonSchemaModel.originalInput, placeholderModel, isNullable);
    alreadySeenModels.set(jsonSchemaModel, metaModel);
  ...
  const metaModel = new ArrayModel(name, jsonSchemaModel.originalInput, valueModel, isNullable);
  alreadySeenModels.set(jsonSchemaModel, metaModel);
