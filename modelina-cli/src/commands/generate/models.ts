...
export default class Models extends ModelinaCommand {
  ...
  async run() {
    ...
    /*
    TEMPORARY SOLUTION: FIND EXACT THING TO PASS AS DOCUMENT; 
    In Async API CLI, `document` was generated by parser as 
    ```
    ...
    ```
    And then it was converted as:
    ```
    const convertedDoc = ConvertDocumentParserAPIVersion(document.json(), 1);
    ```
    But here we have removed parser, so what should we use in place of this?
    */
    const document = "";
