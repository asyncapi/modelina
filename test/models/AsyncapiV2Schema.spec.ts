import {AsyncapiV2ExternalDocumentation, AsyncapiV2Schema} from '../../src/models/AsyncapiV2Schema';

describe('AsyncapiV2Schema', () => {
  describe('toSchema', () => {
    test('should throw error when trying to convert non-object', () => {
      const expectedError = 'Could not convert input to expected copy of AsyncapiV2Schema';
      expect(() => { AsyncapiV2Schema.toSchema(1 as any); }).toThrow(expectedError);
    });
    ...
    test('should handle external documentation', () => {
      const doc = { type: 'string', properties: { test: { type: 'string' } }, externalDocs: { description: 'test' } };
      const d = AsyncapiV2Schema.toSchema(doc) as AsyncapiV2Schema;
      expect(typeof d).toEqual('object');
      const d2 = AsyncapiV2Schema.toSchema(d as any) as AsyncapiV2Schema;
      expect(typeof d2).toEqual('object');
      expect(d.externalDocs instanceof AsyncapiV2ExternalDocumentation).toEqual(true);
    ...
    test('should never return the same instance of properties', () => {
      ...
      expect((d2.properties!['test'] as AsyncapiV2Schema).$id).not.toEqual('test');
    });
