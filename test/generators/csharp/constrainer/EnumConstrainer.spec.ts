...
import { defaultEnumKeyConstraints, ModelEnumKeyConstraints, DefaultEnumKeyConstraints } from '../../../../src/generators/csharp/constrainer/EnumConstrainer';
describe('EnumConstrainer', () => {
  ...
  describe('enum keys', () => {
    ...
    test('should not render number as start char', () => {
      const constrainedKey = CSharpDefaultConstraints.enumKey({enumModel, constrainedEnumModel, enumKey: '1'});
    ...
    test('should use constant naming format', () => {
      const constrainedKey = CSharpDefaultConstraints.enumKey({enumModel, constrainedEnumModel, enumKey: 'some weird_value!"#2'});
    ...
    test('should never render reserved keywords', () => {
      const constrainedKey = CSharpDefaultConstraints.enumKey({enumModel, constrainedEnumModel, enumKey: 'return'});
      expect(constrainedKey).toEqual('RESERVED_RETURN');
    });
  });
  describe('enum values', () => {
    test('should render string values', () => {
      const constrainedValue = CSharpDefaultConstraints.enumValue({enumModel, constrainedEnumModel, enumValue: 'string value'});
