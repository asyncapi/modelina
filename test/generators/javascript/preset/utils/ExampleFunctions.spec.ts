...
import { CommonModel, ConstrainedAnyModel, ConstrainedArrayModel, ConstrainedBooleanModel, ConstrainedIntegerModel, ConstrainedReferenceModel, ConstrainedStringModel, ConstrainedTupleModel, ConstrainedTupleValueModel, ConstrainedUnionModel } from '../../../../../src/models';
describe('Example preset', () => {
  describe('.renderValueFromModel()', () => {
    test('should render refs correctly', () => {
      const refModel = new ConstrainedAnyModel('SomeOtherModel', undefined, 'SomeOtherModel');
    ...
    describe('types', () => {
      ...
      test('Should use first value if there is more then one', () => {
        ...
        const model = new ConstrainedUnionModel('test', undefined, '', [booleanModel, stringModel]);
        const output = renderValueFromModel(model);
        expect(output).toEqual('true');
      });
      describe('array', () => {
        test('should render multiple array values', () => {
          ...
          const model = new ConstrainedTupleModel('test', undefined, '', [new ConstrainedTupleValueModel(0, stringModel), new ConstrainedTupleValueModel(1, integerModel)]);
          const output = renderValueFromModel(model);
