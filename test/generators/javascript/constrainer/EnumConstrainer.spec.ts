...
import { defaultEnumKeyConstraints, ModelEnumKeyConstraints, DefaultEnumKeyConstraints } from '../../../../src/generators/javascript/constrainer/EnumConstrainer';
describe('EnumConstrainer', () => {
  ...
  describe('enum keys', () => {
    test('should never render special chars', () => {
      const constrainedKey = JavaScriptDefaultConstraints.enumKey({enumModel, constrainedEnumModel, enumKey: '%'});
      expect(constrainedKey).toEqual('PERCENT');
    });
    test('should not render number as start char', () => {
      const constrainedKey = JavaScriptDefaultConstraints.enumKey({enumModel, constrainedEnumModel, enumKey: '1'});
    ...
    test('should not contain duplicate keys', () => {
      ...
      const constrainedKey = JavaScriptDefaultConstraints.enumKey({enumModel, constrainedEnumModel, enumKey: 'TEST'});
    ...
    test('should use constant naming format', () => {
      const constrainedKey = JavaScriptDefaultConstraints.enumKey({enumModel, constrainedEnumModel, enumKey: 'some weird_value!"#2'});
    ...
    test('should never render reserved keywords', () => {
      const constrainedKey = JavaScriptDefaultConstraints.enumKey({enumModel, constrainedEnumModel, enumKey: 'return'});
      expect(constrainedKey).toEqual('RESERVED_RETURN');
    });
  });
  describe('enum values', () => {
    test('should render string values', () => {
      const constrainedValue = JavaScriptDefaultConstraints.enumValue({enumModel, constrainedEnumModel, enumValue: 'string value'});
      expect(constrainedValue).toEqual('"string value"');
    });
    test('should render boolean values', () => {
      const constrainedValue = JavaScriptDefaultConstraints.enumValue({enumModel, constrainedEnumModel, enumValue: true});
      expect(constrainedValue).toEqual('"true"');
    });
    test('should render numbers', () => {
      const constrainedValue = JavaScriptDefaultConstraints.enumValue({enumModel, constrainedEnumModel, enumValue: 123});
      expect(constrainedValue).toEqual('123');
    });
    test('should render object', () => {
      const constrainedValue = JavaScriptDefaultConstraints.enumValue({enumModel, constrainedEnumModel, enumValue: {test: 'test'}});
      expect(constrainedValue).toEqual('\'{"test":"test"}\'');
    });
    test('should render unknown value', () => {
      const constrainedValue = JavaScriptDefaultConstraints.enumValue({enumModel, constrainedEnumModel, enumValue: undefined});
