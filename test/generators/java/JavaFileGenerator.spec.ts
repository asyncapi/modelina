...
describe('JavaFileGenerator', () => {
  ...
  describe('generateToFile()', () => {
    ...
    test('should throw accurate error if file cannot be written', async () => {
      ...
      jest.spyOn(generator, 'generateFull').mockResolvedValue([new OutputModel('content', new CommonModel(), '', new CommonInputModel(), [])]);
    
      await expect(generator.generateToSeparateFiles(doc, '/test/', {packageName: 'SomePackage'})).rejects.toEqual(expectedError);
