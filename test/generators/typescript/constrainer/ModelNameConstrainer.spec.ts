...
import { DefaultModelNameConstraints, defaultModelNameConstraints, ModelNameConstraints } from '../../../../src/generators/typescript/constrainer/ModelNameConstrainer';
describe('ModelNameConstrainer', () => {
  test('should never render special chars', () => {
    const constrainedKey = TypeScriptDefaultConstraints.modelName({modelName: '%'});
    expect(constrainedKey).toEqual('Percent');
  });
  test('should never render number as start char', () => {
    const constrainedKey = TypeScriptDefaultConstraints.modelName({modelName: '1'});
  ...
  test('should use constant naming format', () => {
    const constrainedKey = TypeScriptDefaultConstraints.modelName({modelName: 'some weird_value!"#2'});
    expect(constrainedKey).toEqual('SomeWeirdValueExclamationQuotationHash_2');
  });
  test('should never render reserved keywords', () => {
    const constrainedKey = TypeScriptDefaultConstraints.modelName({modelName: 'return'});
