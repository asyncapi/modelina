...
import { defaultEnumKeyConstraints, ModelEnumKeyConstraints, DefaultEnumKeyConstraints } from '../../../../src/generators/typescript/constrainer/EnumConstrainer';
describe('EnumConstrainer', () => {
  ...
  describe('enum keys', () => {
    test('should never render special chars', () => {
      const constrainedKey = TypeScriptDefaultConstraints.enumKey({enumModel, constrainedEnumModel, enumKey: '%'});
      expect(constrainedKey).toEqual('PERCENT');
    });
    test('should not render number as start char', () => {
      const constrainedKey = TypeScriptDefaultConstraints.enumKey({enumModel, constrainedEnumModel, enumKey: '1'});
    ...
    test('should not contain duplicate keys', () => {
      ...
      const constrainedKey = TypeScriptDefaultConstraints.enumKey({enumModel, constrainedEnumModel, enumKey: 'TEST'});
    ...
    test('should use constant naming format', () => {
      const constrainedKey = TypeScriptDefaultConstraints.enumKey({enumModel, constrainedEnumModel, enumKey: 'some weird_value!"#2'});
    ...
    test('should never render reserved keywords', () => {
      const constrainedKey = TypeScriptDefaultConstraints.enumKey({enumModel, constrainedEnumModel, enumKey: 'return'});
      expect(constrainedKey).toEqual('RESERVED_RETURN');
    });
  });
  describe('enum values', () => {
    test('should render string values', () => {
      const constrainedValue = TypeScriptDefaultConstraints.enumValue({enumModel, constrainedEnumModel, enumValue: 'string value'});
      expect(constrainedValue).toEqual('"string value"');
    });
    test('should render boolean values', () => {
      const constrainedValue = TypeScriptDefaultConstraints.enumValue({enumModel, constrainedEnumModel, enumValue: true});
      expect(constrainedValue).toEqual('"true"');
    });
    test.skip('should render numbers', () => {
      const constrainedValue = TypeScriptDefaultConstraints.enumValue({enumModel, constrainedEnumModel, enumValue: 123});
      expect(constrainedValue).toEqual(123);
    });
    test.skip('should render object', () => {
      const constrainedValue = TypeScriptDefaultConstraints.enumValue({enumModel, constrainedEnumModel, enumValue: {test: 'test'}});
      expect(constrainedValue).toEqual('"{\\"test\\":\\"test\\"}"');
    });
    test.skip('should render unknown value', () => {
      const constrainedValue = TypeScriptDefaultConstraints.enumValue({enumModel, constrainedEnumModel, enumValue: undefined});
